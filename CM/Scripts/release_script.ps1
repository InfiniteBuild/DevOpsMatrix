...
& $glabExe release create $env:CI_COMMIT_TAG --ref $env:CI_COMMIT_SHA --name "$projectName V$version"

Write-Host "Release created for tag $env:CI_COMMIT_TAG."

Write-Host "Creating Archives"
& $zipTool a $zipOutDir\DevOpsMatrix_$version.zip $env:pubRelDir\DevOpsMatrix\**

Write-Host "Upload zip files"
& $glabExe release upload $env:CI_COMMIT_TAG "$zipOutDir\DevOpsMatrix_$version.zip#DevOpsMatrix#package"
