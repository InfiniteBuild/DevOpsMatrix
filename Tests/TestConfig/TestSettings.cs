...
namespace TestConfig
{
    public class TestSettings
    {
        ...
        private void LoadSettings()
        {
            ...
            XmlNode privateSettings = xmlDoc.DocumentElement.SelectSingleNode("PrivateSettings");
            if (privateSettings != null)
            {
                if (File.Exists(privateSettings.InnerText))
                {
                    XmlDocument privXmlDoc = new XmlDocument();
                    privXmlDoc.Load(privateSettings.InnerText);
                    Load(privXmlDoc.DocumentElement);
                    return;
                }
            }
